...
int SD(Graph g, int v, int z){
    ...
    //the visited array is the total number of steps from the origin (v) to the current node 

    while (!isEmptyQueue(q)) {
        w = dequeue(&q);

        for (int i=0; i<g.V; i++) {
            if (g.matrix[w-1][i] == 1 && g.visited[i] == 0 && i != v-1) {
                enqueue(&q, i+1);
                g.visited[i] = g.visited[w-1] + 1; //use w-1 because it is the PARENT node
                if (i+1 == z) return g.visited[i]; //reached the target node already, can return the no of steps
            }
        }
    }

    if (g.visited[z-1] == 0) return -1;
